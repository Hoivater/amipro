...
use limb\app\modules\site\quickscheme\core as QS;
 ...
 class Core
    {
        ...
        {
        	$si = new Base\SearchInq("jilert32dwd_account");
			$si -> selectQ();
        ...
        public function nextCommand($message, $history)
        {
        	...
        	if(isset($this -> ini[$history["command"]]["text".($i+2)]))
	        {
	        	...
	        	if($history["command"] == "registration")
	        	{
	        	    ...
	        	    $ri = new Base\RedactionInq("jilert32dwd_account", $table_key);
	        	    $times = time();
	        	    ...
	        	    $ri2 = new Base\RedactionInq("jilert32dwd_user", $table_key_2);
			        ...
			        $sms_rev = "Регистрация завершена. Можно войти на свою страницу https://gifit.ru, используя имя@gifit.ru, как Email для входа на сайт. Вы также можете через меню бота указать настоящий email.";
			        $this -> deleteHistory();
	        	}
	        	...
	        	elseif($history["command"] == "quickscheme")
	        	{
                    $name_old = $this -> nameUserOld();
                    
                    $count_prub = $history["data"][0];
                    $name_s = $history["data"][1];
                    $name_scheme = $history["data"][2];
                    $string_number = $history["data"][3];

                    #посчитать, записать в таблицу, вывести
                    $Scheme = new QS\Scheme($count_prub, $name_s, $name_scheme, $string_number);
	        	    $scheme = $Scheme -> BuildScheme();
	        	    $sms_rev .= $scheme."";
                    
                    $link = Control\Generate::linkgenerate($name_scheme).time();
	        	    ...
	        	    $value = "'".$id."', '". $this -> id_author."', '".$name_scheme."', '".$link."', '".$scheme."', '".$string_number."', '', '', '', '', '', '', '', '', '', '".time()."'";
			        $ri = new Base\RedactionInq($name77656756, $table_key757658);
	        	    $ri -> insert($value);
	        	    $info = parse_ini_file(__DIR__."/../../../setting.ini");
		            $name_site = $info["name_site"];
	        	    $sms_rev .= "Схема доступна по адресу: ".$name_site."/".$name_old."/scheme/".$link;
	        	    $this -> deleteHistory();
	        	}
	        	
	        	elseif($history["command"] == "schemesave")
	        	{
                    $name_old = $this -> nameUserOld();
                    
                    $count_prub = 1;
                    $name_s = "сбн";
                    $name_scheme = $history["data"][0];
                    $string_number = $history["data"][1];
	        	    ...
	        	    $info = parse_ini_file(__DIR__."/../../../setting.ini");
		            $name_site = $info["name_site"];
	        	    $sms_rev .= "Схема доступна по адресу: ".$name_site."/scheme/".$link;
	        	    $this -> deleteHistory();
	        	}
	        	
	        	elseif($history["command"] == "scheme")
	        	{
                    $name_old = $this -> nameUserOld();
	        	...
	        	elseif($history["command"] == "newnick")
	        	{
                    ...
                    else
                    {
    	        	    ...
    	        	    $table_key_2 = "`id`, `name`, `email`, `password`, `access_user`, `code_email`, `code`, `date`";
    	        	    $ri2 = new Base\RedactionInq("jilert32dwd_user", $table_key_2);
    	        	    $ri2 -> update("email", mb_strtolower(trim($history["data"][0]))."@gifit.ru", "name",  $name_old);
        ...
        public function nameUserOld()
        {
            ...
            $si2 = new Base\SearchInq("jilert32dwd_account");
            $si2 -> selectQ();
        ...
        public function runCommand($command)
        {
        	...
        	else
        	{
        	    if($command == "report")
        	    {
        	        $si = new Base\SearchInq("jilert32dwd_tags");
        	        $si -> selectQ();
					...
					{
					    $string_tags = "Вы пока ни одного тега не создали. Это можно сделать на сайте https://gifit.ru";
					}
        ...
        {
    		$si = new Base\SearchInq("jilert32dwd_history");
			$si -> selectQ();
        ...
        {
			$name77656756 = 'jilert32dwd_bloop';
