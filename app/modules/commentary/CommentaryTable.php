<?
	...
	class CommentaryTable
	{
		...
		public $name = 'frfaccr848er4_commentary';//имя таблицы которое используется по умолчанию
		...
		{
			$name77656756 = 'frfaccr848er4_commentary';
			...
			self::iterationCom($id_article);

		}
		public static function iterationCom($id)
		{
			$name77656756 = 'frfaccr848er4_note';
			$table_key757658 = "`id`, `name`, `name_article`, `link`, `text_article`, `tags`, `foto`, `visible`, `count_commentary`, `level`, `status`, `date_start`, `date_end`";
			...
			if(isset($rese[0]["id"])){
				$count = $rese[0]["count_commentary"];
				$count += 1;

				$ri = new Base\RedactionInq($name77656756, $table_key757658);
				$result = $ri -> update("count_commentary", $count, "id", $id);
			}
		...
		public function renderCommentary($id, $auth = "noauth"){
			...
			$limb = new Worker\Limb();
			$si2 = new Base\SearchInq("frfaccr848er4_commentary");
			$si2 -> selectQ();
			$si2 -> whereQ("id_article", $id, "=");
			$si2 -> andQ("levels", 0, "=");
			$si2 -> orderAscQ("date_creation");
			$si2 -> resQ();
