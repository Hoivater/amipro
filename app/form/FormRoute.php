...
require "../../autoload.php";
	...
	class FormRoute extends FormBase
	{
		...
		private function routeF($name_form)
		{
			...
			elseif($name_form == 'new_foto')
			{
				$this -> result = Site\SchemeTable::NewFile($this -> data);
			}

			elseif($name_form == 'newProc')
			{
				if(isset($this -> data["memo"]))
				{
					$this -> result = Site\MemoTable::insertMemo($this -> data);
				}
				else
				{
					$this -> result = Site\BloopTable::insertMessage($this -> data);
				}
			}
		}
