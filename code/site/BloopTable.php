<?
	...
	class BloopTable
	{
		public $tmpltBloop = ['%id%', '%verif_id%', '%name_article%', '%link%', '%description%', '%text_article%', '%reply%', '%tags%', '%category%', '%foto%', '%visible%', '%popular%', '%count_commentary%', '%date%'];//массив из таблиц
		public $resultBloop;//финишная сборка для шаблона для возврата в _Page
		public $name = 'jilert32dwd_bloop';//имя таблицы которое используется по умолчанию
		public $table_key = "`id`, `verif_id`, `name_article`, `link`, `description`, `text_article`, `reply`, `tags`, `category`, `foto`, `visible`, `popular`, `count_commentary`, `date`";
		protected $language;
		#public $replace = [$id, $verif_id, $name_article, $link, $description, $text_article, $reply, $tags, $category, $foto, $visible, $popular, $count_commentary, $date];
		...
		public static function insertFieldLimb($num)
		{
			...
			$name_article = Control\Generate::varchargenerate(255);
			$link = Control\Generate::varchargenerate(255);
			$description = Control\Generate::varchargenerate(255);
			$text_article = Control\Generate::textgenerate();
			$reply = Control\Generate::textgenerate();
			$tags = Control\Generate::textgenerate();
			$category = Control\Generate::textgenerate();
			$foto = Control\Generate::varchargenerate(255);
			$visible = Control\Generate::intgenerate(11);
			$popular = Control\Generate::intgenerate(11);
			$count_commentary = Control\Generate::intgenerate(11);
			$date = Control\Generate::intgenerate(11);
			$value = "'".$id."', '".$verif_id."', '".$name_article."', '".$link."', '".$description."', '".$text_article."', '".$reply."', '".$tags."', '".$category."', '".$foto."', '".$visible."', '".$popular."', '".$count_commentary."', '".$date."'";
